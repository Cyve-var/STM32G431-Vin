...
#define NEOPIXEL_WS2812B
	...
	#define NEO_PIXEL_HIGH_TIME ((TIMER3_PERIOD_RELOAD_VALUE * 1UL) / 2UL)
	#define NEO_PIXEL_LOW_TIME ((TIMER3_PERIOD_RELOAD_VALUE * 3UL) / 10UL)
#endif

#ifdef NEOPIXEL_WS2812B
	#define NEO_PIXEL_HIGH_TIME ((TIMER3_PERIOD_RELOAD_VALUE * 85UL ) / 125UL)
	#define NEO_PIXEL_LOW_TIME  ((TIMER3_PERIOD_RELOAD_VALUE * 40UL ) / 125UL)
#endif
...
unsigned char NeoPixelLedMatrixBrightness;
...
void NeoPixelMain(void) {
	...
	const tLedMatrix* pNeoPixelLedMatrix;
	pNeoPixelLedMatrix = LedMatrixGetMatrix();
	NeoPixelLedMatrixBrightness = LedMatrixGetBrightness();
	for (PixelIndex = 0; PixelIndex < NEOPIXEL_NUM_PIXEL_IN_MATRIX; PixelIndex++) {
		BrightnessRed = pNeoPixelLedMatrix->Pixel[PixelIndex].LedRed;
		BrightnessGreen = pNeoPixelLedMatrix ->Pixel[PixelIndex].LedGreen;
		BrightnessBlue = pNeoPixelLedMatrix->Pixel[PixelIndex].LedBlue;
		if (1 == BrightnessRed) { BrightnessRed = NeoPixelLedMatrixBrightness; }
		if (1 == BrightnessGreen) { BrightnessGreen = NeoPixelLedMatrixBrightness; }
		if (1 == BrightnessBlue) { BrightnessBlue = NeoPixelLedMatrixBrightness; }
